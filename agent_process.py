    ...
    def gather_data(self):
        ...
        for i in range(self._n_games):
            ...
            for timestep in itertools.count():
                ...
                if reward == 1:
                    for j in range(timestep - last_scoring):
                        batch_values.append(1)
                    last_scoring = timestep

                batch_states.append([state])
                batch_actions.append(action)
                # If the agent hasn't scored in MAX_IDLE_STEPS steps, we reset the
                # environment.
                if done or (timestep - last_scoring >= MAX_IDLE_STEPS):
                    for j in range(timestep - last_scoring - 1):
    ...
    def run(self):
        ...
        def treat_queue():
            ...
            if msg == "load":
                ...
                print(
                    f"Process {self._id} loaded the master (0) model."
                )
            ...
            if msg[0] == "train_with_batchs":
                ...
                delta = time.time() - t0
                print(
                    f"Master process finished training. Time : {delta}s \n"
                )
